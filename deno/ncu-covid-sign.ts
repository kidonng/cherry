...
;(async () => {
	const infoResponse = await fetch(`${endpoint}getPreSignInfo`, init)
	const {code, data} = await infoResponse.json()
	...
	const resultResponse = await fetch(`${endpoint}signIn`, {
		...init,
		body: new URLSearchParams(form),
	})
	const result: Record<string, unknown> = await resultResponse.json()
