...
;(async () => {
  ...
  const redirects = [
     ...
     */
    [
      () => hostname.endsWith('.github.com') && document.querySelector(`a[href="//${hostname.replace('.com', '.io')}/"]`),
    ...
    [
      // @ts-expect-error
      () => hostname.endsWith('.m.wikipedia.org') && !navigator.userAgentData?.mobile,
    ...
    [
      // @ts-expect-error
      () => hostname === 'mzh.moegirl.org.cn' && !navigator.userAgentData?.mobile && !searchParams.has('mobileaction'),
    ...
    [
      'strongloop.com',
      { href: 'https://github.com/strongloop' }
    ]
  ]

  for (const [condition, action] of redirects) {
    if (typeof condition === 'function' ? !condition() : hostname !== condition) continue
